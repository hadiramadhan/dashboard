...
const overviewData = [
	{ name: "Revenue", value: "RP 50.000.000", change: 12.5, icon: DollarSign },
...
const OverviewCards = () => {
  return (
    <div className="grid gird-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        {overviewData.map((item, index) => (
            ...
            >
                <div className="flex items-center justify-between">
                    ...
                    <div className={`p-3 rounded-full ${item.change >= 0 ? "bg-green-500 bg-opacity-20" :"bg-red-600 bg-opacity-20" }`}>
                        <item.icon
                        className={`size-6 ${item.change >= 0 ? "text-green-400": "text-red-600"}`}
                        />
                    </div>
                </div>
                <div className={`mt-4 flex items-center ${item.change >= 0 ? "text-green-500" : "text-red-500"}`}>
                    ...
                    <span className="ml-2 text-sm text-gray-400">Last Period</span>
                </div>
            </motion.div>
        ))}
    </div>
